...
// in an console.log box. This saves you from having to hit Return a ton 
...
function log(message) {
  if (!_log_timeout) _log_timeout = window.setTimeout(dump_log, 1000);
...
function inspect(obj) {
  ...
  if (obj) {
    for (var i in obj) {
      if ((obj[i] instanceof Function) || (obj[i] == null) || (i.toUpperCase() == i)) continue;
      message += i + ', ';
    }
    message = message.substr(0, message.length - 2);
  } else message = 'Object is null';
  log(message);
}
